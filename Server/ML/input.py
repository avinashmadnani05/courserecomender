...
def hybrid_recommend(user_id, num_recs=10):
    # Get the user's interactions with courses (collaborative filtering part)
    user_interactions = interaction_matrix.loc[user_id].fillna(0)  # Get user's interactions and fill NaN with 0
    
    # Get predicted ratings using content-based filtering (TF-IDF cosine similarity)
    predicted_ratings_cbf = {}
    user_interests = users_df[users_df['userId'] == user_id]['interests'].values[0]
    user_vector = vectorizer.transform([user_interests])
    ...
    predicted_ratings = {}
    for course_id in courses_df['course_id']:
        # Collaborative filtering rating (user's past interactions with the course)
        collaborative_rating = user_interactions[course_id]  # Either 0 or 1 (since it's an interaction matrix)
        content_based_rating = predicted_ratings_cbf[course_id]
        
        # Weighted average: adjust weights to your preference
        predicted_rating = (0.5 * collaborative_rating) + (0.5 * content_based_rating)
        predicted_ratings[course_id] = predicted_rating
...
# Example usage: Save the recommendations for a specific user
recommended_courses = hybrid_recommend(user_id=1, num_recs=10)
save_recommendations_to_mongo(user_id=1, recommendations=recommended_courses)
